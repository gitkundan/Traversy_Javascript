...
function doChina() {
    ...
    if (zhQ) {
        ...
        switch (dir) {
            case '': case '/activity': case '/algebra': case '/calculus': case '/data': case '/geometry': case '/measure': case '/money': case '/numbers': case '/physics': case '/sets': transQ = true
                break
            default:
        }
        if (transQ) {
...
function fixSpells(elem, tp) {
    ...
    for (let i = 0; children[i]; ++i) {
        ...
        switch (node.nodeType) {
            case Node.ELEMENT_NODE: fixSpells(node, tp)
                break
...
function URLEncode(text) {
    let SAFECHARS = '0123456789' + 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' + 'abcdefghijklmnopqrstuvwxyz' + "-_.!~*'()"
    let HEX = '0123456789ABCDEF'
    let s = ''
    for (let i = 0; i < text.length; i++) {
        let ch = text.charAt(i)
        if (ch == ' ') { s += '+' } else if (SAFECHARS.indexOf(ch) != -1) { s += ch } else {
            let charCode = ch.charCodeAt(0)
            if (charCode > 255) { s += '+' } else {
                s += '%'
...
function menuHTML(typ) {
    ...
    if (typ == 1) {
        s += '<ul role="list" class="bga1" style="display:inline-block; padding:6px; border-radius:10px; ">'
        let ht = 36
...
function menuShow() {
    ...
    if (menuVis) { div.style.display = 'none' } else {
...
function adCodeGet(src) {
    let s = ''
    switch (src) {
        case '300x250': s += '<img src="../images/style/300x250.jpg" alt="dummy" width="300" height="250">'
            break
...
function mainDo() {
    ...
    } else { setTimeout(adsenseDo, 1000) }
    adsHideSet()
    doLocal()
...
function vidHTML(id, titleid = 'title', spanid = 'video', style = 'span') {
    ...
    let s = ''
    switch (style) {
        case 'span': s += '<a href="javascript:vidDo(\'' + id + "','" + spanid + '\')">'
